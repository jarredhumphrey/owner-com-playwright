...
test.describe("Owner.com navigation", () => {
  ...
  test("Navigate between company links", async ({ page }) => {
    ...
    expect(page).toHaveURL("/partner-request");
  });
  ...
  test("Navigate between resources links", async ({ page }) => {
    ...
    expect(page).toHaveURL("/comparison");
  });
});
